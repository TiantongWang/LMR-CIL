...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan  6 21:35:21 2021

@author: tiantong

Using CNN LSTM to extract high level features and then analize it.
"""
...
# parameter configuration of CNN LSTM
nStep = 5
nLength = 10
...
for train_ix, test_ix in kfold.split(X):
	# define model
    model = defineCNNLSTMModel(nLength, nChannel, len(CLASS_FOR_TRAINING))
    ...
    # prepare the data shape to feed into the model
    trainX = trainX.reshape((trainX.shape[0], nStep, nLength, nChannel))
    testX = testX.reshape((testX.shape[0], nStep, nLength, nChannel))
...
# prepare the data shape to feed into the model
entireX = entireX.reshape((entireX.shape[0], nStep, nLength, nChannel))
